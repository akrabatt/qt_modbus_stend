 ...
 * @param modbusrtu_ptr
 * @param test_board_ptr
 * @param cond_var даной переменной выбирем массив МУПСов с питанием (0 - 18v 1 - 24v 2 - 28v)
 * @return
 */
std::map<int, mops>test_board::read_mops_status_return(modbusRTU *modbusrtu_ptr, test_board *test_board_ptr, int cond_var)
{
    // создаем локальный контейнер
    std::map<int, mops> local_mops_map;

    // очищаем наш список с объектами мопсов
    MACRO_SET_MOPS_OBJ_ARR_GLOB(cond_var).clear();
    ...
    {
        // считываем данные i-ого МОПСа
        std::vector<uint16_t> current_buffer = modbusrtu_ptr->mbm_03_read_registers(MACRO_SET_MOPS_18_24_28_ADDR(cond_var)[i], this->mops_quant_reg);
 ...
 * @param modbusrtu_ptr
 * @param test_board_ptr
 * @param cond_var данной переменной выбираем массив МУПСов спитанием (0 - 18v 1 - 24v 2 - 28v)
 * @return
 */
std::map<int, mups>test_board::read_mups_status_return(modbusRTU *modbusrtu_ptr, test_board *test_board_ptr, int cond_var)
{
    // создаем локальный контейнер
    std::map<int, mups> local_mups_map;

    // очищаем наш список с объектами мопсов
    MACRO_SET_MUPS_OBJ_ARR_GLOB(cond_var).clear();
    ...
    for(int i = 0; i < mups_addr_start_reg_size; i++)
    {
        ...
        std::vector<uint16_t> current_buffer = modbusrtu_ptr->mbm_03_read_registers(MACRO_SET_MUPS_18_24_28_ADDR(cond_var)[i], this->mups_quant_reg);
